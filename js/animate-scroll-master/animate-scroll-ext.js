...
(function($, window, document) {
    ...
    var $window = $(window),
        ...
        animateScroll = {
            ...
            bind: function(el, options) {
                $(el).on('none reverse play', function(evt) {
                    if (evt.type == 'none') {
                        return;
                    }
                    ...
                    if (!!$timeline && evt.type != action) {
                        ...
                        if ($this.css("visibility") == "hidden") {
                            ...
                            if ($this.data('tween')) {
                                $this.data('tween').progress(1, false);
                            }
                        }
            ...
            check: function(target) {
                ...
                var inview = (vTop < elBottom && elBottom < vBottom) || (vTop < elTop && elTop < vBottom);
                if (inview) {
                    if (tObj.data("sort-play")) {
                        tObj.data("sort-play")();
                    }
                }
                return inview ? 'reverse' : 'none';
            },
                    ...
                    if (!!resize) {
                        setTimeout(function() {
                            ...
                            if ($this.hasClass("autoPlay")) {
                                $this.trigger(animateScroll.check($this));
                            }
                        }, 250);
                    } else {
                        // trigger animation event
    ...
    $.fn.animateScroll = function(opts) {
        ...
        if (this[0] != document) {
            ...
            if (animateScroll.animations && animateScroll.animations.length > 0) {
                animateScroll.init(animateScroll.animations, options);
            }
        }
    }

    $.fn.animateScrollExt = function(opts) {
        this.addClass("autoPlay");
        //vars
        if (this[0] != document) {

            if (!animateScroll.animations) {
                animateScroll.animations = $();
            }

            if (animateScroll.animations) {
                this.each(function(){
                    if (opts) {
                        $(this).attr("data-animate-scroll", JSON.stringify(opts));
                    }
                    animateScroll.animations.push(this);
                });
            }
        } else {
            if (animateScroll.animations && animateScroll.animations.length > 0) {
    ...
    $.fn.sortAnimateScroll = function($child) {

        $child.animateScrollExt();
        $child.removeClass("autoPlay");

        this.each(function() {
            $(this).data("sort-play", function() {
                $child.each(function() {
                    var This = this;
                    setTimeout(function() {
